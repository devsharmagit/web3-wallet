...
setIsAlertOpen(false)
  ...
  >
    ...
    <div className="h-[88vh] flex justify-center items-center py-4">
      ...
      <div className='max-w-7xl w-[100vw] px-4  flex  h-full border-2 rounded-2xl bg-black bg-opacity-30 no-scrollbar border-white border-opacity-30 overflow-scroll '>
...
<div className='flex flex-col gap-2'>
    {walletState?.wallets?.map((wallet, index)=>{
      return <div onClick={()=>setActiveWallet(wallet)}  key={wallet?.publicId} className={`px-2 py-1 bg-white bg-opacity-20 cursor-pointer border rounded-md text-sm border-white border-opacity-20 text-white ${wallet?.publicId === activeWallet?.publicId && "!bg-white bg-opacity-100 !text-black"}`}>
Account {index + 1}
      </div>
    })}
